...
public class RemedioDAO {
    ...
    public RemedioTO findByCodigo(Long codigo){
        ...
        try(PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql))
        {
            ...
            System.out.println("Erro ao consultar um remédio da tabela: " + e.getMessage());
        } finally {
            ConnectionFactory.closeConnection();
        }
        return remedio;
    ...
    public boolean delete(Long codigo){
        ...
        try(PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)){
            ps.setLong(1, codigo);
            return ps.executeUpdate() > 0; //substitui o if/else para o return, retorn true se for executado (modificar alguma linha) ou retorna false se tive 0 linhas modificadas
        }catch (SQLException e) {
            System.out.println("Erro ao deletar um remédio da tabela: " + e.getMessage());
        } finally {
            ConnectionFactory.closeConnection();
        }
        return false;
    ...
    public RemedioTO update(RemedioTO remedio){
        ...
        try(PreparedStatement ps = ConnectionFactory.getConnection().prepareStatement(sql)){
            ...
            System.out.println("Erro ao atualizar um remédio da tabela: " + e.getMessage());
        } finally {
            ConnectionFactory.closeConnection();
        }
        return null;
